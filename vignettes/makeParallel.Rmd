...
coresGenerate <- function(schedule, mc.cores = 2L, ...)
{
    ...
    # generated otherwise. Passing `file = FALSE` ensures we don't write
    # to a file.
...
Building on the previous example, we can define a class `WorkerMapSchedule`
...
Since we've already defined `coresGenerate` above we can just wrap that.
