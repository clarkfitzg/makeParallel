...
TODO: Maybe do a couple different vignettes- one more conceptual, one end
user focused.
...
parallel workers as specified by the option `mc.cores` in the parallel
package. We can do that as follows:

```{r}

coresGenerate <- function(schedule, MC_CORES = 2L, ...)
{
    # Rely on the method dispatch for the actual work.
    out <- generate(schedule, ...)

    # Construct an expression containing the desired code, in this case
    # something along the lines of:
    # options(mc.cores = 2L)
    setCores <- substitute(options(mc.cores = MC_CORES)
                          , list(MC_CORES = MC_CORES))

    # Combine the newly constructed expression with what would have been
    # generated otherwise and assign it back into the object.
    out@code <- c(setCores, writeCode(out, FALSE))
    out
}
...
programming system. For example, we can define a scheduling algorithm that
explicitly sets the number of workers. We can do this by extending
`MapSchedule` and building on the functionality that already exists.

```{r}
setClass("WorkerMapSchedule", c(mc.cores = "integer"), contains = "MapSchedule")

workerMapSchedule = function(graph, mc.cores = 2L, ...)
{
    message("Do something fancy here.")
