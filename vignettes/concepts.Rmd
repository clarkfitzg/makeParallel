...
I'll call it 'standard form', so I can use a more common word.

The main thing I want right now from the modified code is to group plotting edges.
We could also change `for` loops -> `lapply` at this step.
In the future I could see wanting more, for example, breaking down subexpressions, and scheduling those individually.

All of these things will require us to change the timing information.

For scheduling we only need the modified code, and the graph that comes from the modified code.
