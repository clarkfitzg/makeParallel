...
interfaces. The conventional approach has advantages and disadvantages.
Well defined interfaces are helpful for understanding and for debugging.

<!--
Parallelism is used to make code run faster. The standard advice for making
code run faster is:

1. Write simple code that does what you want
2. Profile it to see what's slow
3. Make the slow parts faster
-->

This fits in well with the conventional approach to optimizing code for
performance, namely to first write a simple, expressive version and then 

Profiling shows where the program is slow so that the prog
The programmer can profile to determine where 
...
initialize clusters of workers communicating over sockets.

The problem with this is that then this code becomes
