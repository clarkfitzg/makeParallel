...
This is a package to take general R code and transform it into parallel
code. This differs from most approaches to parallel programming in R. Most
...
1. Non standard evaluation (NSE) makes static code analysis more difficult
   ...
   The CodeDepends package supports this by providing [function
   handlers](https://github.com/duncantl/CodeDepends/blob/master/R/functionHandlers.R),
   which essentially collect the usage information in a custom way for each
   function with NSE. Thus we need to know exactly what each of these
   functions does.
