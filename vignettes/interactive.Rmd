...
The `parallel_evaluator` was designed to support the following specific use
case (if you use it for other purposes, drop me a line I'm curious to
hear about it).
...
you're doing could happen in parallel on the parts, but you don't want to
worry about all the issues that can arise with parallel code when you're in
the middle of your interactive analysis. You want a system to manage all
the details of the parallelism for you, so that you can focus on the
analysis. Let the `parallel_evaluator` manage the details.
...
manager process then the parallel evaluator will export it to the cluster,
...
The parallel evaluator allows us to execute the same code that is
