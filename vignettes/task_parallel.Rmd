...
1. There is a script that the user would like to run multiple times
2. The execution time for the parallel paths does not vary wildly
...
1. run the script once, measuring time required to evaluate each
   ...
   serialized
2. infer the dependency structure of the code, ie. where task independent
   threads are
