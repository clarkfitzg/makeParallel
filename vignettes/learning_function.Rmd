...
Then we can pass both implementations into `learn_func` and let it figure
it out for us.
...
ols = learn_func(ols_naive, ols_clever)
...
- `ols()` will try different implementations
- `ols()` times itself
...
microbenchmark(ols_clever(X, y), ols_naive(X, y), times = 10)
