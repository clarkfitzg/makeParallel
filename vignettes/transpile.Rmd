...
the column indices instead. Because not every text file has column names.

For the moment I'm ignoring NSE such as `subset`.

One approach is to take all the uses of column names and map them into
integers. The code will go through three representations then:
...
`data.frame` subsetting commands into single `[`.
